...
__host__ void CudaInitSpParams(superpixel_params* sp_params, const int s_std, const int i_std, const int nSPs){
  int num_block = ceil( double(nSPs) / double(THREADS_PER_BLOCK) ); 
...
__global__ void InitSpParams(superpixel_params* sp_params, const int s_std, const int i_std, const int nSPs)
{
  // the label
...
__global__ void GetImageCartoon(uchar3* image_mean_gpu, const int* seg, const superpixel_params* sp_params, const int nPixels){
  ...
  double L = sp_params[k].mu_i.x;
  double La = sp_params[k].mu_i.y;
  double Lb = sp_params[k].mu_i.z;
